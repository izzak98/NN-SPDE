def gen_dgm_params(trial):
    ...
    hidden_dims = [trial.suggest_categorical(
        f"n_params_{i}", [4, 8, 16, 32, 64, 128, 256]) for i in range(n_hidden)]
    ...
    params = {
        ...
        "output_activation": trial.suggest_categorical("output_activation", ["tanh", ""])
    }
    return params
...
def gen_mim_params(trial):
    ...
    u_hidden_dims = [trial.suggest_categorical(
        f"n_u_params_{i}", [4, 8, 16, 32, 64, 128, 256]) for i in range(n_u_hidden)]
    n_p_hidden = trial.suggest_int("n_p_hidden", 1, 5)
    p_hidden_dims = [trial.suggest_categorical(
        f"n_p_params_{i}", [4, 8, 16, 32, 64, 128, 256]) for i in range(n_p_hidden)]
    ...
    params = {
        ...
        "p_out_activation": trial.suggest_categorical("p_output_activation", ["tanh", ""]),
